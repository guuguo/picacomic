        ...
        if (e != null) {
            when (e) {
                ...
                is HttpException -> {
                    ...
                    try {
                        val errorRes: ResponseModel<String> = myGson.fromJson(body?.string(), object : TypeToken<ResponseModel<String>>() {}.type)
                        if (errorRes.code != 200) {
                            onApiLoadError(errorRes.message + " " + errorRes.detail)
                            return
                        }
                    } catch (ie: Exception) {
